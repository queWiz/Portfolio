...
export function ProjectShowcase() {
  ...
  // Auto-advancing timed slideshow with smooth progress bar
  useEffect(() => {
    if (isPaused) return;
  ...
  return (
    ...
    >
      {/* 
        TOP / TABS ROW: Inspired by Image 1 (Timed progress bars with decimal numbering)
        Responsive: Touch-friendly swipeable tabs on mobile, full grid on desktop
      */}
        ...
        {PROJECTS.map((proj, idx) => {
          ...
          return (
            <button
              ...
              className={`text-left p-3 sm:p-4 rounded-2xl border transition-all duration-300 relative overflow-hidden flex flex-col justify-between shrink-0 sm:shrink min-w-[145px] sm:min-w-0 snap-start min-h-[82px] sm:min-h-[90px] ${
                isActive
                  ? "bg-white dark:bg-[#111622] border-cobalt/60 shadow-md ring-1 ring-cobalt/20"
            ...
            >
              ...
              {/* Timed Horizontal Progress Bar (fills when active) */}
              <div className="w-full h-1 bg-slate-100 dark:bg-white/[0.05] rounded-full overflow-hidden mt-3">
                {isActive ? (
                  <div
                    className="h-full bg-cobalt transition-all duration-75 ease-linear rounded-full"
                    style={{ width: `${progress}%` }}
                  />
                ) : (
                  <div
                    className={`h-full ${
                      idx < currentIndex ? "bg-slate-300 dark:bg-white/20" : "w-0"
                    }`}
                  />
                )}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
          ...
          <div className="lg:col-span-6 flex flex-col gap-5 sm:gap-6">
            <div>
              ...
              <h3 className="text-2xl sm:text-3xl lg:text-4xl font-heading font-extrabold text-slate-900 dark:text-white tracking-tight leading-snug">
          ...
          {/* Right Column: Web & Mobile Mockup Frame (6 cols) */}
...
function DesktopBrowserMockup({ project }: { project: ProjectData }) {
  return (
    <div className="w-full max-w-full rounded-2xl bg-slate-900 text-slate-100 border border-slate-800 shadow-2xl overflow-hidden font-sans">
      ...
      <div className="p-3.5 sm:p-6 bg-slate-900 min-h-[290px] sm:min-h-[340px] flex flex-col justify-between">
        <div>
          <div className="flex flex-wrap items-center justify-between gap-1 pb-2.5 sm:pb-3 mb-3 sm:mb-4 border-b border-slate-800">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mb-3 sm:mb-4">
            <div className="p-2 sm:p-3 rounded-xl bg-slate-950/70 border border-slate-800">
          ...
          <div className="p-2.5 sm:p-4 rounded-xl bg-slate-950 border border-slate-800/80 space-y-1.5 font-mono text-[10px] sm:text-[11px] text-slate-300">
    ...
    <div className="w-full max-w-[260px] sm:max-w-[280px] rounded-[36px] bg-slate-950 border-[5px] sm:border-[6px] border-slate-800 shadow-2xl overflow-hidden font-sans p-2 mx-auto">
      ...
      <div className="rounded-[24px] bg-slate-900 p-4 min-h-[360px] flex flex-col justify-between text-slate-100">
        <div>
          ...
          <div className="p-2.5 rounded-lg bg-slate-950 border border-slate-800 mb-3 space-y-1">
          ...
          <div className="p-2.5 rounded-lg bg-slate-950 border border-slate-800 text-[10px] font-mono text-slate-400 leading-snug">
            {project.overview.slice(0, 110)}...
          </div>
        </div>

        {/* Mobile App Navigation Simulation */}
        <div className="pt-3 border-t border-slate-800/80 flex items-center justify-around text-[10px] font-mono text-slate-400">
